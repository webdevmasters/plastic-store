...
@section('title','Izmena promocije')

@section('content')
    <div class="container c1691">
        <div class="row c1859">
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-4 col-12 c1867">
                <h4 data-type="header" id="ibxf1lw">IZMENA PROMOCIJE</h4>
                <hr>
                @if(session('message'))
                    <div class="alert alert-success">{{ session('message') }}</div>
                @endif

                <form enctype="multipart/form-data" id="i10l" method="post" name="form" action="{{route('admin.promotions.update',$promotion->id)}}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row"><label class="col-form-label col-sm-3" for="category">Kategorija :</label>
                        <div class="col-sm-9">
                            <select class="selectpicker form-control" id="category" name="category" value="{{ $promotion->category->id }}">
...
@push('scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#category').val(@json($promotion->category->id));
